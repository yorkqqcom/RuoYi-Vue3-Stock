-- ----------------------------
-- Tushare数据下载模块菜单配置（PostgreSQL版本）
-- ----------------------------

-- 主菜单：Tushare数据管理
insert into sys_menu values(2000, 'Tushare数据管理', 0, 5, 'tushare', null, '', '', 1, 0, 'M', '0', '0', '', 'guide', 'admin', current_timestamp, '', null, 'Tushare数据管理目录');

-- 二级菜单：接口配置管理
insert into sys_menu values(2001, '接口配置', 2000, 1, 'apiconfig', 'tushare/apiconfig/index', '', '', 1, 0, 'C', '0', '0', 'tushare:apiConfig:list', 'list', 'admin', current_timestamp, '', null, 'Tushare接口配置菜单');
